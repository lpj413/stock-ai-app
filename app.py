...
# 3. 標題與時間判定
st.title("🍎 股市投資小幫手")
st.subheader("AI 雙價位全方位分析 (終極穩定版)")
...
if analyze_btn:
    ...
    with st.spinner('數據同步與強制對齊中...'):
        try:
            ...
            if hist.empty:
                st.error("找不到該代號數據，請確認輸入格式（如：2324.TW）")
                st.stop()

            # B. 抓取最新成交價 (解決 27.45 顯示問題)
            try:
            ...
            # D. 強制數據對齊 (解決 Index Out of Bounds 的核心)
            combined_df = pd.DataFrame(index=hist.index)
            combined_df['TW_Raw'] = hist['Close']
            combined_df['TW_Adj'] = adj_hist['Close']
            combined_df['ADR'] = df_adr
            combined_df['IDX'] = df_idx

            # 關鍵：先填補缺失值，再剔除無法對齊的行
            df = combined_df.ffill().dropna()

            if len(df) < 20:
                st.error("美股與台股數據對齊失敗，請稍後再試。")
                st.stop()
            ...
            # 若 Raw 已更新但 Adj 還沒，依比例補償
            if curr_raw != df['TW_Raw'].iloc[-1]:
                ratio_val = df['TW_Raw'].iloc[-1] / df['TW_Adj'].iloc[-1]
                curr_adj = curr_raw / ratio_val
            ...
            if is_long_bull and is_strong_up:
                ...
                st.info(f"⚡【短線反彈】{config['name']} 長線偏弱，僅為反彈。")
                advice_msg = "賺了就跑，不要長抱。"
            else:
            ...
            st.write(f"### 🚩 實戰參考 (市場掛牌價)")
            col_a, col_b = st.columns(2)
            col_a.info(f"📍 **建議下單(地板)：{lower_band:.2f}**")
            col_b.error(f"📍 **建議獲利(天花板)：{upper_band:.2f}**")

            with st.expander("🔍 數據細節"):
                st.write(f"市場價: {curr_raw:.2f} / 還原價: {curr_adj:.2f}")
                st.write(f"股利價差: {curr_raw - curr_adj:.2f}")
                st.markdown(f"**操作建議：**\n{advice_msg}")

        except Exception as e:
            st.error(f"系統暫時無法對齊數據，請重新點擊分析按鈕。({e})")import streamlit as st
import yfinance as yf
import pandas as pd
from sklearn.ensemble import RandomForestClassifier, RandomForestRegressor
from datetime import datetime
