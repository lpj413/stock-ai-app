...
# 1. 網頁頁面設定
st.set_page_config(page_title="AI 雙軌戰略導航儀", page_icon="📈", layout="centered")
...
if analyze_btn:
    ...
    with st.spinner('正在同步全球數據並強制校準價格...'):
        try:
            # A. 數據抓取
            ticker = yf.Ticker(target)
            
            # 抓取歷史數據包 (包含原始 Close 與 Adj Close)
            hist = ticker.history(period="2y", auto_adjust=False)
            adj_hist = ticker.history(period="2y", auto_adjust=True)
            ...
            # 如果數據源依然顯示 26.95，嘗試從即時數據抓取真正的市場價
            if curr_raw == curr_adj and target == "2324.TW":
                # 這是一個針對仁寶數據異常的硬補丁
                # 嘗試抓取今日的 Open 或 High/Low 均值作為掛牌價參考
                curr_raw = float(hist['Open'].iloc[-1]) if hist['Open'].iloc[-1] > curr_raw else curr_raw
            ...
            if is_long_bull and is_strong_up:
                ...
                st.warning("⏳【長多短空】趨勢未壞，但短線偵測到回檔壓力。")
